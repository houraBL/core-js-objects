...
class SelectorBuilder {
  ...
  element(value) {
    ...
    if (
      this.idValue ||
      this.classValues.length > 0 ||
      this.attrValue ||
      this.pseudoClassValues.length > 0 ||
      this.pseudoElementValue
    ) {
      throw new Error(
  ...
  attr(value) {
    if (this.pseudoClassValues.length > 0 || this.pseudoElementValue) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
  ...
  static combine(selector1, combinator, selector2) {
    const combined = new SelectorBuilder();
    combined.combination = [selector1, selector2];
